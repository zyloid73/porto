...
export function Random() {
  return (
    <section className="random-scene page-gutter" aria-labelledby="random-title">
      <div className="random-topline mono"><span>INTERLUDE / A FAMILIAR STORY</span><PanelsTopLeft size={23} strokeWidth={1} aria-hidden="true" /></div>
      ...
      <span className="random-footnote mono muted">SOME THINGS NEVER CHANGE.</span>
    </section>
  )
}
...
export function FindMe() {
  ...
  return (
    <section id="find-me" className="find-scene page-gutter" data-scene="find-me" tabIndex={-1} aria-labelledby="find-title">
      ...
      <div className="find-layout"><div className="find-heading"><h2 id="find-title" className="display">FIND<br />ME<span className="blue">↗</span></h2><p>Probably online.<br />Definitely in another tab.</p>{socialLinks.some((social) => !social.url) && <span className="mono muted">PERSONAL LINKS COMING SOON.</span>}</div>
...
export function Farewell() {
  ...
  return (
    <footer id="farewell" className="farewell-scene page-gutter" tabIndex={-1}>
      <div className="farewell-top"><span className="mono muted">THAT&apos;S ME, FOR NOW.</span><Asterisk size={55} strokeWidth={0.9} aria-hidden="true" /></div>
      <p className="farewell-title display">SEE YOU<br /><span className="farewell-last-line">AROUND<span className="blue">.</span><span className="farewell-thanks">Thanks for<br />visiting.</span></span></p>
      <div className="farewell-bottom mono"><span>RANO / 2026</span><span className="footer-note muted">MADE WITH CURIOSITY. AND TOO MANY TABS.</span><a href="#intro" onClick={(event) => { event.preventDefault(); navigate("#intro") }}>BACK TO TOP <ArrowUp size={16} strokeWidth={1.3} aria-hidden="true" /></a></div>
    </footer>
  )
}
